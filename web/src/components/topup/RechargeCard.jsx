                  ...
                  <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2'>
                    {presetAmounts.map((preset, index) => {
                      ...
                      const actualPay = Number(
                        presetPayAmounts?.[preset.value] ?? 0,
                      );
                      const hasActualPay =
                        Number.isFinite(actualPay) && actualPay > 0;
                      const hasDiscount =
                        discount > 0 && discount < 1.0 && hasActualPay;
                      const originalPay = hasDiscount
                        ? actualPay / discount
                        : actualPay;
                      const save = hasDiscount ? originalPay - actualPay : 0;
                      ...
                      return (
                        ...
                        >
                          <div style={{ textAlign: 'center' }}>
                            ...
                            >
                              <Coins size={18} />
                              {renderQuotaWithAmount(preset.value)}
                              {hasDiscount && (
                            ...
                            >
                              {hasActualPay
                                ? `${t('实付')} ${actualPay.toFixed(2)} ${t('元')}，${t('节省')} ${save.toFixed(2)} ${t('元')}`
                                : `${t('实付')} -- ${t('元')}，${t('节省')} -- ${t('元')}`}
                            </div>
                          </div>
                        </Card>
