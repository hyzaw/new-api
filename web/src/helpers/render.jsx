...
export function parseTiersFromExpr(exprStr) {
  ...
  try {
    ...
    const condGroup = `((?:(?:p|c|len|\\(?\\s*p\\s*\\+\\s*cr\\s*\\)?)\\s*(?:<|<=|>|>=)\\s*[\\d.eE+]+)(?:\\s*&&\\s*(?:p|c|len|\\(?\\s*p\\s*\\+\\s*cr\\s*\\)?)\\s*(?:<|<=|>|>=)\\s*[\\d.eE+]+)*)`;
    ...
    while ((m = tierRe.exec(body)) !== null) {
      ...
      if (condStr) {
        for (const cp of condStr.split(/\s*&&\s*/)) {
          const cm = cp.trim().match(/^(p|c|len|\(?\s*p\s*\+\s*cr\s*\)?)\s*(<|<=|>|>=)\s*([\d.eE+]+)$/);
