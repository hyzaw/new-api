...
- 2026-05-05: 真实手动 `/compact` 请求的另一处缺口在 `setting/billing_setting`：基础模型若配置的是 `tiered_expr`，`gpt-5.5-openai-compact` 之前不会继承该计费模式和表达式，于是 `ModelPriceHelper` 不会走表达式计费分支，最终落到“价格未配置”。
- 2026-05-05: 线上第二段 `new_api_panic` 实际还受 `main.go` 里的 `gin.CustomRecovery` 影响；之前仅修 `middleware/recover.go` 不足以覆盖当前服务实例。
