...
func GetAllUsers(c *gin.Context) {
	...
	sortBy := c.Query("sort")
	users, total, err := model.GetAllUsers(pageInfo, sortBy)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func SearchUsers(c *gin.Context) {
	...
	sortBy := c.Query("sort")
	pageInfo := common.GetPageQuery(c)
	users, total, err := model.SearchUsers(keyword, group, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), sortBy)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func ManageUser(c *gin.Context) {
	...
	case "promote_root":
		if myRole != common.RoleRootUser {
			common.ApiErrorI18n(c, i18n.MsgUserAdminCannotPromoteRoot)
			return
		}
		if user.Role >= common.RoleRootUser {
	...
	if req.Action == "disable" || req.Action == "promote" || req.Action == "promote_root" || req.Action == "demote" {
		if err := model.InvalidateUserCache(user.Id); err != nil {
...
type UpdateUserSettingRequest struct {
	...
	RecordIpLog                      bool    `json:"record_ip_log"` // 兼容旧前端入参，后端始终强制开启
}
