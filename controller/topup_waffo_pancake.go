...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	if errMsg := validateTopupAmount(req.Amount, int64(setting.WaffoPancakeMinTopUp)); errMsg != "" {
...
func getWaffoPancakePayMoney(amount int64, group string) float64 {
	...
	payMoney := dAmount.
		...
		Mul(decimal.NewFromFloat(getTopupDiscount(amount)))
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	completed, err := model.RechargeWaffoPancake(tradeNo)
	if err != nil {
	...
	if !completed {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 跳过重复完成 trade_no=%s event_id=%s order_id=%s client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
		c.String(http.StatusOK, "OK")
		return
	}
