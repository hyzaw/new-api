...
func getWaffoPayMoney(amount float64, group string) float64 {
	...
	discount := getTopupDiscount(int64(originalAmount))
...
func RequestWaffoAmount(c *gin.Context) {
	...
	if errMsg := validateTopupAmount(req.Amount, waffoMinTopup); errMsg != "" {
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	...
	completed, err := model.RechargeWaffo(merchantOrderId, c.ClientIP())
	if err != nil {
	...
	if completed {
		service.NotifyTopupSuccessAsync(merchantOrderId, c.ClientIP(), "waffo")
	}
