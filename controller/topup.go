...
func GetTopUpInfo(c *gin.Context) {
	...
	enableAlipayF2F := setting.AlipayF2FEnabled &&
		setting.AlipayAppID != "" &&
		...
		if topUp.Status == common.TopUpStatusPending {
			if topUp.PaymentMethod != verifyInfo.Type {
				...
				if err := topUp.Update(); err != nil {
					logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 更新实际支付方式失败 trade_no=%s user_id=%d client_ip=%s error=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), err.Error()))
				}
			}
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	completed, err := model.ManualCompleteTopUp(req.TradeNo, c.ClientIP())
	if err != nil {
		common.ApiError(c, err)
		return
	}
	if completed {
		service.NotifyTopupSuccessAsync(req.TradeNo, c.ClientIP(), "admin_manual")
	}
