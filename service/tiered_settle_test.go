...
func TestBuildTieredTokenParams_Claude_AggregateCacheCreationCountsAsCC(t *testing.T) {
	usage := &dto.Usage{
		PromptTokens:     6,
		...
		PromptTokensDetails: dto.InputTokenDetails{
			CachedTokens:         25329,
			CachedCreationTokens: 6846,
			TextTokens:           6,
		},
	}
	expr := `tier("base", p * 5 + c * 25 + cr * 0.5 + cc * 6.25)`
	got := tieredQuota(expr, usage, true, 0.3)
	// Aggregate cache creation falls back to cc (5m) when Claude does not
	// return a cache_creation 5m/1h split.
	want := ((6*5 + 282*25 + 25329*0.5 + 6846*6.25) / 1_000_000.0) * testQuotaPerUnit * 0.3
	if math.Abs(got-want) > 0.01 {
		t.Fatalf("quota = %f, want %f", got, want)
	}
	...
	if params.CC != 6846 {
		t.Fatalf("CC = %f, want 6846", params.CC)
	}
	if params.CC1h != 0 {
		t.Fatalf("CC1h = %f, want 0", params.CC1h)
	}
}
...
func TestBuildTieredTokenParams_Len_GPT(t *testing.T) {
	...
	usedVars := billingexpr.UsedVars(`tier("base", p * 2.5 + c * 15 + cr * 0.25)`)
...
func TestBuildTieredTokenParams_Len_Claude(t *testing.T) {
	...
	usedVars := billingexpr.UsedVars(`tier("base", p * 3 + c * 15 + cr * 0.3 + cc * 3.75 + cc1h * 6)`)
	params := BuildTieredTokenParams(usage, true, usedVars)
...
const complexTieredExpr = `len <= 200000 ? tier("standard", p * 3 + c * 15 + cr * 0.3 + cc * 3.75 + cc1h * 6 + img * 3 + img_o * 30 + ai * 10 + ao * 40) : tier("long_context", p * 6 + c * 22.5 + cr * 0.6 + cc * 7.5 + cc1h * 12 + img * 6 + img_o * 60 + ai * 20 + ao * 80)`
