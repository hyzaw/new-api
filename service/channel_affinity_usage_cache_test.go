...
func newChannelAffinityStatsIDs(t *testing.T) (string, string) {
	t.Helper()
	seq := channelAffinityUsageCacheTestSeq.Add(1)
	safeName := strings.ReplaceAll(t.Name(), "/", "_")
	return fmt.Sprintf("%s_rule_%d_%d", safeName, time.Now().UnixNano(), seq),
		fmt.Sprintf("%s_fp_%d_%d", safeName, time.Now().UnixNano(), seq)
}

func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	ruleName, keyFP := newChannelAffinityStatsIDs(t)
	usingGroup := "default"
